...
module.exports = function compileSchema(userSchema, options) {
  ...
  try {
    for (var _iterator = userTypeMap.values()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function getDirectives(def) {
  return (def.directives || []).reduce((acc, dir) => _objectSpread({}, acc, {
    [dir.name.value]: dir.arguments.filter(isArgument).reduce((args, arg) => _objectSpread({}, args, {
...
function convertBlockType(def, map, options, directives) {
  ...
  if (inlineTypes) {
    ...
    const ofDef = convertArrayField(inlineTypes, map, _objectSpread({}, options, {
      parent: def
    }), type, inlineTypes);
    ofMembers = ofDef.of;
  }
...
function getAnnotationMembers(def, annotations, map, options) {
  return convertArrayField({}, map, _objectSpread({}, options, {
    parent: def
  }), {
    kind: 'NamedType',
    name: {
...
function convertObjectType(def, map, options) {
  ...
  return withoutUndefined({
    ...
    description: def.description && def.description.value,
    icon: icons[display.icon],
    fields: def.fields.map(field => convertField(field, map, _objectSpread({}, options, {
      parent: def
    }))),
...
function convertArrayField(def, map, options, type, field) {
  ...
  return withoutUndefined(_objectSpread({}, field, {
    type: 'array',
    of: ofDef
  }));
}
...
function convertNamedTypeField(def, map, options, type, baseField) {
  ...
  if (!referencedType && !isCoreScalar(typeName)) {
    ...
    return withoutUndefined(_objectSpread({}, baseField, getFieldDefForNamedType(type)));
  }
  ...
  return withoutUndefined(_objectSpread({}, baseField, fieldDef, refOverride));
}
